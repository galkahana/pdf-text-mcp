...
- **Current Phase**: Phase 4 - Monorepo Restructure ✅ **COMPLETE**
...
### 📋 Next Session: Phase 5.5 - Python Agent Integration Fix

**Priority**: Fix before Phase 6 (detour to ensure end-to-end functionality)

**Goal**: Fix PydanticAI example-agent integration with MCP protocol

**Known Issue**:
- MCP server works perfectly (verified with direct HTTP/SSE calls via curl)
- Python agent fails with Gemini schema validation error:
  ```
  Error: 10 validation errors for GenerateContentConfig
  tools.0.Tool.function_declarations.0.parameters.oneOf
    Extra inputs are not permitted
  ```
- Root cause: MCP tool schemas don't match Gemini's expected function calling format
- Affects both stdio and HTTP/SSE transports

**Investigation Steps**:
1. **Debug MCP Schema Output**:
   - Check what the MCP server returns for `tools/list`
   - Inspect the schema format from `extract_text` and `extract_metadata` tools
   - Compare with Gemini's expected function calling schema

2. **Test Alternative Models**:
   - Try with Claude API (may have better MCP compatibility)
   - Try with OpenAI API (test if issue is Gemini-specific)
   - Isolate whether it's PydanticAI, MCP client, or Gemini integration

3. **PydanticAI MCP Client**:
   - Check PydanticAI's MCP client implementation
   - Look for known issues with MCP tool schema translation
   - Check if there are configuration options for schema format

4. **Potential Solutions**:
   - Update MCP tool schemas to be more compatible
   - Switch to a different model that handles MCP tools better
   - Fix/patch PydanticAI's MCP-to-Gemini translation
   - Document workarounds if no clean fix exists

**Starting Point**:
- Create new branch: `fix/python-agent-integration`
- Start with debugging MCP schema output
- Test with alternative models
- Document findings and implement fix

---

### 📋 Future: Phase 6 - Observability & Operations

**Goal**: Add production-ready monitoring, logging, and metrics (AFTER agent fix)
